...
def main():
    ...
    for ADDR in ADDRS:
        ...
        try:
            ...
            mailFunction(f"socket error - {host}", f"error connecting managment socket server ({ADDR[0][0]}:{ADDR[0][1]}) from {host} at LOGIN attempt for {user}\n{str(e)}")
            os.system(f'pkill -KILL -u {user}')

        print(f"[CONNECTED] VIS CLIENT TO MGMT SERVER AT {ADDR[0][0]}:{ADDR[0][1]}")
