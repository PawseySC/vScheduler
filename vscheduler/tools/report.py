...
class Process(multiprocessing.Process):
    ...
    def run(self):
        ...
        # if (config.partition['windows']['node'] in self.hostname and int(self.hostname.replace(config.partition['windows']['node'], "")) in MyCredentials.windows_booking_range) \
        #     or (config.partition['linux']['node'] in self.hostname and int(self.hostname.replace(config.partition['linux']['node'], "")) in MyCredentials.linux_booking_range):
        ...
        # elif (config.partition['windows']['node'] in self.hostname and int(self.hostname.replace(config.partition['windows']['node'], "")) in MyCredentials.windows_general_range) \
        #     or (config.partition['linux']['node'] in self.hostname and int(self.hostname.replace(config.partition['linux']['node'], "")) in MyCredentials.linux_general_range):
