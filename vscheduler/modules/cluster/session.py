...
def session(node: str, user) -> str:
    ...
    try:
        ...
        for line in stdout:
            second = line.strip('\n')
            print (f"second: {second}") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
        ...
        print (f"could not connect to < {node} > to query user session time") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
