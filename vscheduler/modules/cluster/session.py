...
def session(node: str, user) -> str:
    ...
    try:
        ...
        for line in stdout:
            ...
            logger_win.info (f"second: {second}") if MyCredentials.windows_node_name in node else logger_unix.info (f"second: {second}")
        my_connection.close()
        return second

    except:
        print (f"could not connect to < {node} > to query user session time") if MyPrintCondition.fprint else 0
        logger_win.error (f"could not connect to < {node} > to query user session time") if MyCredentials.windows_node_name in node else logger_unix.error (f"could not connect to < {node} > to query user session time")