...
def empty_pool_connection(node, user, pool):
    logger_win.info (f"node: < {node} >, user: < {user} >, pool: < {pool} >") if config.partition['windows']['node'] in node else logger_unix.info (f"node: < {node} >, user: < {user} >, pool: < {pool} >")
    ...
    logger_win.info (f"node_entity of < { node } >: < {node_entity} >") if config.partition['windows']['node'] in node else logger_unix.info (f"node_entity of < { node } >: < {node_entity} >")
    logger_win.info (f"pool_entity of < { pool } >: < {pool_entity} >") if config.partition['windows']['node'] in node else logger_unix.info (f"pool_entity of < { pool } >: < {pool_entity} >")
    logger_win.info (f"user_entity of < { user } >: < {user_entity} >") if config.partition['windows']['node'] in node else logger_unix.info (f"user_entity of < { user } >: < {user_entity} >")
    ...
    if node_entity is not None and pool_entity is not None:
        ...
        logger_win.critical (f"NoneType object is not subscriptable, node_entity is empty") if config.partition['windows']['node'] in node else logger_unix.critical (f"NoneType object is not subscriptable, node_entity is empty")
        exit