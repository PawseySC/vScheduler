...
logger = report_records.log_agent("booked")


def booking_report_generator(username, hostname, start, end):
    """
    Generates bookings report for user(s)
    """
    directory = str(Path.home()) + "/visualisation_scheduler/vscheduler/reports" + str(hostname) + '\\' + str((MyBrackets.utc_now.date() - timedelta(1)).year) + "/" + str((MyBrackets.utc_now.date() - timedelta(1)).month) + '\\' + str((MyBrackets.utc_now.date() - timedelta(1)).day)
    os.makedirs(directory) if not os.path.exists(directory) else 0
    my_html_report = open(directory + "/" + str(MyBrackets.utc_now.date() - timedelta(1)) + ".html", "w")
    logger.info (f"{directory} was created")
    my_txt_report = open(directory + "/" + str(MyBrackets.utc_now.date() - timedelta(1)) + ".txt","w")
    logger.info (f"{directory}/{my_txt_report} created")
    my_cumulative_report = open(directory + "/" + str(MyBrackets.utc_now.date() - timedelta(1)) + ".dat","w")
    logger.info (f"{directory}/{my_cumulative_report} created")
    my_csv_report = open(directory + "/" + str(MyBrackets.utc_now.date() - timedelta(1)) + ".csv","w")
    logger.info (f"{directory}/{my_csv_report} created")
