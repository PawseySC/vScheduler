...
def reservation_instances(start_bracket, end_bracket):
    ...
    try:
        ...
        print (f"\n{tabulate(sentence, headers=['reservation_instance_id', 'start_date', 'end_date', 'series_id'])}") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
        logger.info (f"\n{tabulate(sentence, headers=['reservation_instance_id', 'start_date', 'end_date', 'series_id'])}")
        return reservation_instances_results
    except:
        print ("error retreiving reservation instances") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
