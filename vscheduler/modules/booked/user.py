...
def user_details_print(users_results):
    ...
    print (f"\n{tabulate(sentence, headers=['user_id', 'fname', 'lname', 'username', 'email'])}") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
...
def user_details_by_user_id(logged_in_user):
    try:
        ...
        print (f"Users Error, no record for current logged in user id < {logged_in_user} > in booked") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
...
def user_details_by_username(logged_in_user):
    try:
        ...
        print (f"Users Error, no record for current logged in user < {logged_in_user} > in booked") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
...
def user_details():
    try:
        ...
        print ("Users Error, no record for users in booked") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
