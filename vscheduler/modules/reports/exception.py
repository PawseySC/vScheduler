...
def exception_list(user, start, end):
    ...
    try:
        ...
        exception_list_query = f"SELECT user, start, end, wall_time FROM {config.get("database.report.table.exception")} WHERE"
        if not start and not end:
            exception_list_query = exception_list_query + f"  start = end"
            if user:
            ...
            exception_list_query = exception_list_query + f" start >= '{start}' AND end <= '{end}'"
            if user:
                exception_list_query = exception_list_query + f" AND user = '{user}'"
        elif start and not end:
            exception_list_query = exception_list_query + f" start >= '{start}'"
            if user:
                exception_list_query = exception_list_query + f" AND user = '{user}'"
        elif end and not start:
            exception_list_query = exception_list_query + f" end <= '{end}'"
            if user:
                exception_list_query = exception_list_query + f" AND user = '{user}'"

        print (f"exception_list_query: {exception_list_query}") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
        ...
        [print (f"{user} is not excepted" if len(exception_query_results) == 0 else f"{exception_query_result[0]} is excepted since {exception_query_result[1]} with wall time of {exception_query_result[3]} hours") for exception_query_result in exception_query_results] if verbose.mode else 0 # if MyPrintCondition.fprint else 0
...
def exception_update(user, mode, wall_time):
    ...
    try:
        ...
        if mode == "activate":
            if len(exceptions) > 0:                
                print (f"{user} is already excepted since {exceptions[0][1]} with wall time of {exceptions[0][3]} hours") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
            ...
            if len(exceptions) == 0:
                print (f"{user} is not excepted already") if verbose.mode else 0 # if MyPrintCondition.fprint else 0
        ...
        if exception_update_query:
            ...
            # print (f"exception_update_query: {exception_update_query}") if verbose.mode else 0 # if MyPrintCondition.fprint else 0 
            logger.info (f"exception_update_query: {exception_update_query}")
            print (f"{my_cursor.rowcount} record(s) inserted/updated into exception table") if verbose.mode else 0 # if MyPrintCondition.fprint else 0 
